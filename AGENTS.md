...
- **One analyst LLM** (muse via `ANALYST_*` in `.env`, OpenAI-compatible chat)
  ...
  @h3rkk flagged — it may mirror their calls or trade its own view), RSS news
...
- `TG_BOT_TOKEN` was exposed in an old chat transcript — rotate via BotFather
  if not yet done (STATUS.md follow-up).
...
- Extend test fixtures from reality (real sampled messages), don't invent
