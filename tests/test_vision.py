...
def feed_with(reader_obj, state=None, callers=("caller1",)):
...
def test_captionless_caller_chart_lands_and_wakes():
    ...
    f = feed_with(reader())
    woke = f.ingest(1, time.time(), "caller1", "", image_desc="NVDA 4h, 214.30")
...
def test_a_message_with_neither_text_nor_image_is_still_dropped():
    f = feed_with(reader())
    assert f.ingest(2, time.time(), "caller1", "") is False
...
def test_editing_a_caption_does_not_re_read_the_picture():
    ...
    f = feed_with(reader())
    f.ingest(3, 1000.0, "caller1", "chart", image_desc="BTC 1h, support 108k")
    assert f.ingest(3, 1001.0, "caller1", "chart — long here") is True
...
def test_an_unchanged_replay_is_not_fresh():
    f = feed_with(reader())
    assert f.ingest(4, 1000.0, "caller1", "", image_desc="d") is True
    assert f.ingest(4, 1000.0, "caller1", "", image_desc="d") is False
...
def test_the_prompt_shows_the_image_under_its_message():
    ...
    bundle["telegram"] = [{
        "msg_id": 9, "ts": 990.0, "sender": "caller1", "is_caller": True,
