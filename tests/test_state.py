...
def test_tg_dedupe_and_recent(tmp_path):
    s = mk(tmp_path)
    assert s.add_tg_message(1, 1000.0, "h3rkk", "SOL long", True) is True
    assert s.add_tg_message(1, 1000.0, "h3rkk", "SOL long", True) is False
    assert s.add_tg_message(1, 1002.0, "h3rkk", "SOL long — edited", True) is True
    ...
    assert s.tg_message(1)["sender"] == "h3rkk"
