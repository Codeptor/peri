...
export type SnippetCopyButtonProps = ComponentProps<typeof Button> & {
  asChild?: boolean;  // base-lyra Button has no asChild — declared here for kibo's cloneElement path
  value: string;
  onCopy?: () => void;
...
export const SnippetTabsContent = ({
  ...
  <TabsContent className={cn("mt-0 bg-background p-4 text-sm", className)} {...props} render={<pre className="truncate" />}>{children}</TabsContent>
);
