...
tg_signal_groups = [{ id = -1001234567890, slug = "caller_group", senders = ["caller1"] }]
"""
    p = tmp_path / "t.toml"
    ...
    assert g["slug"] == "caller_group"
    assert g["senders"] == ["caller1"]


def test_should_forward():
    assert tg_news_pipe.should_forward("caller1", ["caller1"]) is True
    assert tg_news_pipe.should_forward("caller1", ["caller1"]) is True
    assert tg_news_pipe.should_forward("other", ["caller1"]) is False
    assert tg_news_pipe.should_forward(None, ["caller1"]) is False
...
def test_group_payload_source_and_url():
    ...
    payload = tg_news_pipe.build_group_payload("caller_group", -1001234567890, 123, 1700000000000, "Setup 2 SOL Short Entry: 75.5 SL: 75.9 TP: 74.6")
    assert payload["source"] == "tg:caller_group"
