...
fn filter_skip_rechecks(
    ...
    candidates.into_iter().filter(|(nominee, forced_by_caller_group)| {
        *forced_by_caller_group || skip_rechecks.get(&nominee.market).is_none_or(|(last_ts, last_score)| {
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            for nominee in forced {
                if let Some((_, forced_by_caller_group)) = candidates.iter_mut().find(|(existing, _)| existing.market == nominee.market) {
                    *forced_by_caller_group = true;
                } else {
            ...
            for (nominee, forced_by_caller_group) in candidates {
                ...
                tokio::spawn(async move {
                    ...
                    let (decision_opt, decision_id, decision_analyst) = if is_stub() {
                        ...
                        if forced_by_caller_group {
                            reason.push_str(" forced:caller_group");
                        }
...
mod tests {
    ...
    async fn manual_halt_refuses_screener_and_force_nominated_entries_first() {
        ...
        assert_eq!(forced.len(), 1, "the caller group call must nominate SOL");
        assert_eq!(
            entry_gate.check("bravo", &forced[0].market, 0.9, now).await,
            Err(GateRefusal::EntriesHalted),
            "force-nominated the caller group candidate must use the same first gate"
        );
    }
