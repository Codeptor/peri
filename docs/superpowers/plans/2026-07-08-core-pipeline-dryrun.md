...
**Goal:** A working daemon that reads the Telegram group "Trencher's Den", filters to @h3rkk, parses his messages into structured trade intents, applies risk checks, and records what it *would* trade — via a dry-run router that places no real orders.
...
- **Signal source:** only messages from allowlisted senders (`@h3rkk`) are ever parsed for action.
...
botta/
  ...
  tests/
    ...
    fixtures/messages.py      # real sampled @h3rkk messages
    test_config.py
    test_models.py
    test_state.py
    test_parser.py
...
[telegram]
group_id = -1001234567890          # Trencher's Den
allowlist = ["h3rkk"]              # usernames (no @) whose calls are acted on
...
def test_load_config(tmp_path, monkeypatch):
    ...
    assert cfg.telegram.allowlist == ["h3rkk"]
...
**Interfaces:**
- Consumes: nothing.
- Produces: `ENTRIES: list[str]`, `MANAGES: list[str]`, `EXITS: list[str]`, `NOISE: list[str]` — verbatim sampled @h3rkk messages (`⏎` replaced with real newlines).

- [ ] **Step 1: Write `tests/fixtures/messages.py`**

```python
# Verbatim samples from Trencher's Den (@h3rkk), newlines restored.
ENTRIES = [
    "SOL Long\n\nTP: mine is 82, conservative can be 79.5\n\nSL: 30% max, i dont have any",
    "HYPE LONG\n\nTP: 70\nbut conservative TP: 68.2\n\nSL: i haven't kept one",
    "SOL LONG\n\nTP: 83.4\nSL: dekhlo jaisa bhi ho",
    "GOLD SHORT\n\nTP: 4144\nSL: 4169",
    "BTC SHORT\n\ncalm down on leverage, can get choppy",
    "SOL Short (Down)\nlow lev",
    "I am going Long on HYPE",
    "SOL Short",
]
MANAGES = [
...
EXITS = [
    "full tp reached gg",
    "booked whole",
    "closed short on SOL. Looks bullish",
    "Closing this.",
...
git commit -m "test: add sampled @h3rkk message fixtures"
```
...
def _deps(tmp_path, mode="dry"):
    cfg = Config(mode=mode,
                 telegram=TelegramCfg(-100, ["h3rkk"], 42),
...
Expected: prints `botta running · mode=dry …`, then `[DRY] Opened …` lines whenever @h3rkk posts an actionable call. Leave running through a couple of real calls; confirm parses + intended actions look right. Ctrl-C to stop.
