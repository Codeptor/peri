...
A always-on service that reads the Telegram group **"Trencher's Den"**, mirrors the
perp calls of one trusted caller (**@h3rkk**), and executes the equivalent positions
...
- **Signal source:** @h3rkk posts structured entries and manages them live. Everyone else
...
- **Call style:** loose, multi-line, heavy Hinglish. Examples:
  - Entry: `SOL LONG` / `TP: 83.4` / `SL: dekhlo jaisa bhi ho` (SL = "keep whatever")
  ...
  - Exit: `full tp reached` / `booked whole` / `closing this`
  - Follow-up TP/SL as a **separate** message shortly after entry: `Conservative TP: 82.52`
- **Images are frequent** (95 of 400 msgs; 55 from @h3rkk; latest 2026-07-08 09:27 UTC).
...
```
Telegram (Trencher's Den)
      │  new/edited messages (text + captions)
      ▼
[Listener]  Telethon user client · filters to @h3rkk · stale-signal guard
